...
export async function getSwaggerDefinitions(source, auth = { mode: AUTH_MODES.NONE }) {
  if (await pathExists(source)) {
    return [];
  }
  ...
  if (looksLikeHtml(raw, contentType)) {
    const inlineConfig = extractSwaggerUiInlineConfig(raw);
    if (inlineConfig?.url || inlineConfig?.urls) {
      configs.push(inlineConfig);
    }

    for (const configUrl of buildSwaggerUiConfigCandidates(source, inlineConfig)) {
      try {
        configs.push(await loadSwaggerUiConfig(configUrl, headers));
      } catch {
        // Definition discovery is best-effort. loadOpenApiDocument handles final errors.
      }
    }
  } else {
    try {
  ...
  return dedupeDefinitions(
    configs.flatMap((config) => buildDefinitionsFromConfig(source, config)),
  );
}
...
async function loadSwaggerUiDocument(source, headers, html = "") {
  ...
  for (const configUrl of buildSwaggerUiConfigCandidates(source, inlineConfig)) {
    try {
      attempts.push(await loadSwaggerUiConfig(configUrl, headers));
    } catch (error) {
      configErrors.push(error.message);
    }
  }

  if (!attempts.length && configErrors.length) {
    console.warn(`Swagger config fetch ishlamadi, fallback ishlatiladi: ${configErrors[0]}`);
  }
...
async function loadSwaggerUiConfig(configUrl, headers) {
...
function buildSwaggerUiConfigCandidates(source, inlineConfig) {
  const pageUrl = new URL(source);
  const candidates = [];

  if (inlineConfig?.configUrl) {
    candidates.push(new URL(inlineConfig.configUrl, pageUrl).toString());
  }

  candidates.push(
    new URL("./swagger-config", pageUrl).toString(),
    new URL("../v3/api-docs/swagger-config", pageUrl).toString(),
    new URL("/v3/api-docs/swagger-config", pageUrl).toString(),
  );
...
function resolveSpecUrlFromConfig(source, config) {
  ...
  return resolveSwaggerRelativeUrl(source, selected.url);
}

function buildDefinitionsFromConfig(source, config) {
  const urls = Array.isArray(config?.urls) ? config.urls : [];
  ...
  if (urls.length) {
    return urls
      ...
      .map((item) => ({
        ...
        url: resolveSwaggerRelativeUrl(source, item.url),
      }));
  }
    ...
    return [
      {
        name: config.url,
        label: config.url,
        url: resolveSwaggerRelativeUrl(source, config.url),
      },
    ];
  }
...
function resolveSwaggerRelativeUrl(source, targetUrl) {
  return new URL(targetUrl, new URL(source)).toString();
}
...
function buildCommonSpecCandidates(source) {
  ...
  const groupCandidates = primaryName
    ? [
        `../v3/api-docs/${encodeURIComponent(primaryName)}`,
        `/v3/api-docs/${encodeURIComponent(primaryName)}`,
        `../v3/api-docs?group=${encodeURIComponent(primaryName)}`,
        `/v3/api-docs?group=${encodeURIComponent(primaryName)}`,
      ]
    : [];
  ...
  return [
    ...new Set(
      [...groupCandidates, ...baseCandidates].map((candidate) =>
