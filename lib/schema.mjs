...
function normalizeNestedSchema(schema, doc) {
  const resolved = dereference(schema, doc) || {};

  if (resolved.type === "array" && resolved.items) {
    return {
      ...resolved,
      items: normalizeNestedSchema(resolved.items, doc),
    };
  }
    ...
    return {
      ...
      properties: Object.fromEntries(
        Object.entries(resolved.properties).map(([key, value]) => [
          key,
          normalizeNestedSchema(value, doc),
        ]),
      ),
    };
  }
...
export function getSchemaFields(schema, doc) {
  ...
  return Object.entries(resolved.properties).map(([name, propertySchema]) => {
    const normalized = normalizeNestedSchema(propertySchema, doc);
    return {
      name,
