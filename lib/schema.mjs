...
export function schemaToTsType(schema, enumInfo = null) {
  ...
  if (enumInfo) {
    return enumInfo.isArray ? `${enumInfo.typeName}[]` : enumInfo.typeName;
  }
...
export function schemaToYupType(schema, enumInfo = null) {
  ...
  if (enumInfo) {
    const validator = `yup.mixed<${enumInfo.typeName}>().oneOf([...${enumInfo.constName}])`;
...
export function mapperExpression(field) {
  ...
  if (field.enumInfo) {
    return field.enumInfo.isArray
      ? `get<${field.enumInfo.typeName}[]>(item, "${path}") ?? []`
      : `get<${field.enumInfo.typeName}>(item, "${path}") ?? ${field.enumInfo.constName}[0]`;
  }
