...
import { singularize, toCamelCase, toKebabCase, toScreamingSnakeCase } from "./naming.mjs";
...
function buildEnumConstName(field) {
  return toScreamingSnakeCase(field.name) || "ENUM_VALUES";
}

function buildEnumInfo({ field, registry }) {
  const enumShape = getEnumShape(field.schema);
  if (!enumShape) {
    return null;
  }

  const baseName = buildEnumConstName(field);
  ...
  while (usedConstNames.has(dedupedBaseName)) {
    counter += 1;
    dedupedBaseName = `${baseName}${counter}`;
  }

  const definition = {
    constName: dedupedBaseName,
...
function withEnumInfo(fields = [], registry) {
  return fields.map((field) => ({
    ...field,
    enumInfo: buildEnumInfo({ field, registry }),
  }));
}
...
function collectEnumImports(fields = [], { enumValueMode = "all" } = {}) {
  const imports = [];
  const seen = new Set();
...
export function buildEnumContext({ schemaContext, extraActions = [] }) {
  const registry = new Map();
  const entityFields = withEnumInfo(schemaContext.entityFields, registry);
  const formFields = withEnumInfo(schemaContext.formFields, registry);
  const enrichedExtraActions = extraActions.map((action) => ({
    ...action,
    requestFields: withEnumInfo(action.requestFields, registry),
  }));
...
export function buildEnumConstantsBlock(enumDefinitions = []) {
  return enumDefinitions
    .map((definition) => `export const ${definition.constName} = ${JSON.stringify(definition.values)} as const;`)
    .join("\n\n");
}
...
export function buildConstantsImportSpec({
  ...
  return [
    ...
    ...collectEnumImports(fields, { enumValueMode }),
  ].join(", ");
}
